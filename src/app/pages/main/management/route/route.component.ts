...
@Component({
  selector: 'app-route',
  imports: [GenericTableComponent, GenericFormComponent, LeafletModule, RouteMapComponent],
...
export class RouteComponent {
  ...
  async handleFormSubmit(formData: FormData): Promise<void> {
    ...
    const routeObj: IMutateRoute = {
      ...
      geojson: JSON.stringify(this.currentCreatedRouteObject  || {}), // Assuming this is the GeoJSON object
      attribute: JSON.stringify({}) // Assuming you want to send an empty attribute object
    }
    try {
...
onGeoJsonImported(geoJsonData: any) {
  console.log('GeoJSON imported successfully:', geoJsonData);
  this.currentCreatedRouteObject = geoJsonData;
  console.log('currentCreatedRouteObject:', this.currentCreatedRouteObject);
